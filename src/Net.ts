...
async function getResponseBody(response: Response) {
  ...
  const body = await (contentType.includes('application/json')
    ? response.json()
    : response.text());

  return contentType.includes('xml')
    ? new window.DOMParser().parseFromString(body, 'text/xml')
    : body;
}

async function fetchCall<Options extends RequestInit>(
  url: string,
